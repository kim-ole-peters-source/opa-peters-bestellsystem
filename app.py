...
# Vercel Functions haben kein dauerhaft beschreibbares Projekt-Verzeichnis.
# Für den kostenlosen Testbetrieb speichern wir Daten deshalb in /tmp.
# Achtung: /tmp ist nicht dauerhaft. Nach Cold Starts/Redeploys können Daten weg sein.
if os.environ.get("VERCEL"):
    DATA_DIR = os.environ.get("DATA_DIR", "/tmp/opa-peters-data")
    os.makedirs(DATA_DIR, exist_ok=True)
    DB_PATH = os.path.join(DATA_DIR, "bestellsystem.db")
    SETTINGS_PATH = os.path.join(DATA_DIR, "settings.json")

    bundled_db = os.path.join(BASE_DIR, "bestellsystem.db")
    bundled_settings = os.path.join(BASE_DIR, "settings.json")
    if not os.path.exists(DB_PATH) and os.path.exists(bundled_db):
        import shutil as _shutil
        _shutil.copyfile(bundled_db, DB_PATH)
    if not os.path.exists(SETTINGS_PATH) and os.path.exists(bundled_settings):
        import shutil as _shutil
        _shutil.copyfile(bundled_settings, SETTINGS_PATH)
else:
    DATA_DIR = BASE_DIR
    DB_PATH = os.path.join(BASE_DIR, "bestellsystem.db")
...
class App(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/orders/"):
            if not self.is_admin():
                return self.redirect("/admin/login")
            return self.serve_file(os.path.join(BASE_DIR, path.lstrip("/")), "application/pdf")
        if path == "/":
